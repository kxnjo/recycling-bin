...
def update_bin_levels():
    ...
    with profile_block("update_bin_levels_total"):
        for key in ['a', 'b', 'c']:
            readings = []
            sensor = sensors[key]

            with profile_block(f"bin_{key}_measure"):
                for _ in range(5):
                    try:
...
def run_sequence(target_angle):
    ...
    try:
        with profile_block("servo_sequence_total", extra=f"target={target_angle}"):

            with profile_block("servo_main_move"):
